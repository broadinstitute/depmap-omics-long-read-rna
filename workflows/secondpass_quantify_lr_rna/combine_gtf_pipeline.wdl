...
task filter_isoquant {
    ...
    Int disk_space = (
        ceil(
        size(raw_gtf, "GiB")) + 20 + additional_disk_gb
    )
...
task run_sqanti3 {
  ...
  # Calculate required disk space based on input file sizes
    Int disk_space = (
        ceil(size(ref_annotation_gtf, "GiB")) + ceil(size(ref_fasta, "GiB"))
...
task gffread {
    ...
    Int disk_space = (
        ceil(
        size(gencode_gtf, "GiB"))  + 20 + additional_disk_gb) 
...
task process_tracking_file {
    ...
    Int disk_space = (
        ceil(
        size(tracking_file, "GiB") * 3)  + 20 + additional_disk_gb
    )
